...
本章節說明如何優化 Uptime Kuma Cluster 以支援 **1000 個以上的監控**而不會有效能問題。
...
| 元件 | 修改前 | 修改後 | 效果 |
|------|--------|--------|------|
| **資料庫連線池** | min: 0, max: 10 | min: 2, max: 50 | 5 倍並行查詢能力 |
| **監控啟動方式** | 逐一啟動 (300-1000ms/個) | 批次並行 (50 個/批) | 啟動時間縮短 ~90% |
| **MariaDB 連線數** | 151 (預設) | 500 | 支援多節點高並行 |
| **InnoDB Buffer Pool** | 預設 | 512M | 提升資料庫效能 |
| **對外 Port** | 3001-3005 | 33001-33010 | 避免 port 衝突 |

#### 第二階段優化（高可用擴展）

| 元件 | 修改前 | 修改後 | 效果 |
|------|--------|--------|------|
| **Redis 快取** | 無（使用 Lua shared_dict） | Redis 7-alpine | 高效能分散式快取 |
| **集群節點數** | 5 節點 | 10 節點 | 2 倍容量 |
| **每節點監控上限** | 2000 | 3000 | 最多 30,000 監控 |
| **MariaDB 讀取複本** | 無 | 1 個讀取複本 | 分散讀取負載 |
| **健康檢查間隔** | 15 秒 | 10 秒 | 更快的故障偵測 |
| **路由快取 TTL** | 600 秒 | 900 秒 (15 分鐘) | 減少資料庫查詢 |
| **Reconciliation 間隔** | 每分鐘 | 每 30 秒 | 更快的監控同步 |

### 📊 效能提升預估

| 指標 | 優化前 | 第一階段 | 第二階段 |
|------|--------|----------|----------|
| **支援監控數** | 5,000 | 10,000 | **30,000** |
| **1000 監控啟動時間** | 5-17 分鐘 | ~40 秒 | **~40 秒** |
| **並行資料庫查詢** | 10 個 | 50 個 | **50 個** |
| **故障偵測速度** | 30 秒 | 15 秒 | **10 秒** |
| **監控同步速度** | 60 秒 | 60 秒 | **30 秒** |
...
| 檔案 | 修改內容 |
|------|--------|
| `server/database.js` | `mariadbPoolConfig.max` 從 10 增加到 50 |
| `server/server.js` | 監控啟動從逐一改為批次並行（50 個/批） |

#### 第二階段

| 檔案 | 修改內容 |
|------|--------|
| `lua/config.lua` | 加入 Redis、讀取複本設定；`node_count=10`、`monitor_limit=3000`、`health_check.interval=10` |
| `lua/redis_cache.lua` | **新增** - Redis 快取模組，用於路由、節點狀態、容量快取 |
| `docker-compose-cluster.yaml` | 加入 Redis、MariaDB 讀取複本、節點 6-10 |
| `server/jobs.js` | Reconciliation 從每分鐘改為每 30 秒（支援環境變數） |
...
```bash
# 在 docker-compose-cluster.yaml 或 .env 檔案中設定

# 集群配置
CLUSTER_NODE_COUNT=10            # 叢集節點數量
MONITOR_LIMIT_PER_NODE=3000      # 每節點最大監控數
HEALTH_CHECK_INTERVAL=10         # 健康檢查間隔（秒）
RECONCILE_INTERVAL_SEC=30        # Reconciliation 間隔（秒）

# Redis 配置
REDIS_ENABLED=true               # 啟用 Redis 快取
REDIS_HOST=redis                 # Redis 主機名
REDIS_PORT=6379                  # Redis 連接埠

# MariaDB 讀取複本
DB_REPLICA_ENABLED=true          # 啟用讀取複本
DB_REPLICA_HOST=mariadb-replica  # 讀取複本主機名
DB_REPLICA_PORT=3306             # 讀取複本連接埠
```
