...
async function sendInfo(socket, hideVersion = false) {
    ...
    if (currentNodeId) {
        ...
        currentNode = await Node.getByNodeId(currentNodeId);
    }
    ...
    socket.emit("info", {
        ...
        dbType,
        currentNodeId,
        currentNode: currentNode ? currentNode.toJSON() : null,
 ...
 * @param {Socket} socket Socket.io socket instance
 * @returns {Promise<Bean[]>} List of nodes
 */
async function sendNodeList(socket) {
    const timeLogger = new TimeLogger();

    const Node = require("./model/node");
    let result = [];
    let list = await Node.getAll();

    for (let bean of list) {
        result.push(bean.toJSON());
    }

    io.to(socket.userID).emit("nodeList", result);

    timeLogger.print("Send Node List");
