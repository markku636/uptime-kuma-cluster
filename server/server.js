...
if (currentNodeId) {
    log.info("server", "Current Node ID: " + currentNodeId);
} else {
    log.info("server", "No Node ID specified - will run all monitors");
}
...
const { sendNotificationList, sendHeartbeatList, sendInfo, sendProxyList, sendDockerHostList, sendAPIKeyList, sendRemoteBrowserList, sendMonitorTypeList, sendNodeList } = require("./client");
...
const { nodeSocketHandler } = require("./socket-handlers/node-socket-handler");
...
(async () => {
    ...
    // REST API Router
    const restApiRouter = require("./routers/rest-api-router");
    ...
    io.on("connection", async (socket) => {
        ...
        // Get monitors from all nodes (for performance, limit to essential data only)
        socket.on("getAllMonitors", async (callback) => {
            try {
                checkLogin(socket);
                
                log.info("monitor", `Get All Monitors Request: User ID: ${socket.userID}`);
                
                // Get all monitors without node filtering, but limit the data for performance
                let query = " user_id = ? ";
                let queryParams = [ socket.userID ];
                
                let monitorList = await R.find("monitor", query + "ORDER BY weight DESC, name", queryParams);
                
                // Prepare minimal preload data for performance
...
async function startMonitors() {
    ...
    let list = await R.find("monitor", whereClause, params);
    
    log.info("server", `Found ${list.length} monitors to start`);
