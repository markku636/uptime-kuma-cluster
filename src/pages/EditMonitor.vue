            ...
            return this.$t("Example:", [ `
{
    "HeaderName": "HeaderValue"
        ...
        /**
         * Generates the node options list for the assigned node dropdown
         * @returns {Array} The node options list
         */
        nodeOptionsList() {
            let list = [
                {
            ...
            console.log("$root.nodeList:", this.$root.nodeList);
            console.log("nodeList type:", typeof this.$root.nodeList);
            console.log("nodeList keys:", this.$root.nodeList ? Object.keys(this.$root.nodeList) : 'null/undefined');
            console.log("nodeList values:", this.$root.nodeList ? Object.values(this.$root.nodeList) : 'null/undefined');
            
            if (this.$root.nodeList && Object.keys(this.$root.nodeList).length > 0) {
                const nodeItems = Object.values(this.$root.nodeList).map(node => {
                    console.log("Processing node:", JSON.stringify(node, null, 2));
                    let label = node.nodeId;
                    if (node.nodeName) {
                        label += ` (${node.nodeName})`;
                    }
                    if (this.$root.info && this.$root.info.currentNodeId && node.nodeId === this.$root.info.currentNodeId) {
                        label += ` - ${this.$t("Current Node")}`;
                    }
                    console.log("Generated label:", label);
                    console.log("Generated value:", node.nodeId);
                    return {
                        label: label,
                        value: node.nodeId,
                    };
                ...
                console.log("Generated nodeItems:", JSON.stringify(nodeItems, null, 2));
                list = list.concat(nodeItems);
            } else {
                console.log("No nodeList or empty nodeList");
            }
            
            console.log("Final list:", JSON.stringify(list, null, 2));
    ...
    watch: {
        ...
        "$root.nodeList": {
            handler() {
                // Force reactivity update when nodeList changes
                this.$forceUpdate();
            },
            deep: true
        },
    ...
    mounted() {
        ...
        // Ensure node list is loaded
        if (!this.$root.nodeList || Object.keys(this.$root.nodeList).length === 0) {
            console.log("NodeList is empty, requesting from server...");
            this.$root.getSocket().emit("getNodeList", (res) => {
                console.log("getNodeList response:", res);
                if (res.ok) {
                    this.$root.nodeList = {};
            ...
            console.log("NodeList already exists:", this.$root.nodeList);
        }
